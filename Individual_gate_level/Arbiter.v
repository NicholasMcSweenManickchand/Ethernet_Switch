module Arbiter( // 1 arbiter per gate so it only handles the message for it
    input [3:0] valid_input, // 0 is for A and 3 is for D // valid input will swallow control_signal and be the only gate keeper, this is the destination port
...
// priority will be given to port A to D initially, then ports will follow this order but only account for who passed the most recently
