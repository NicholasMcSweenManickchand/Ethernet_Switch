...
module Ingress_FCS_Checker(
    input wire clk, 
    input wire reset,
    
    // The incoming data stream (payload + the 4-byte FCS appended to it)
    input wire [7:0] data, 
    
    // High when the stream is actively receiving data
    input wire valid_data, 
    
    // High specifically on the exact clock cycle of the final byte of the frame
    input wire eof, 
    
    // The output flag sent to the egress FCS module to corrupt the outgoing CRC
    output reg poison 
);
    ...
    // --- The CRC32 Combinational Math ---
    // Copied directly from my egress FCS module to ensure the math matches exactly.
    ...
    // --- Sequential Logic & Magic Residue Check ---
    always @(posedge clk) begin
        if (reset) begin
            crcIn <= 32'hFF_FF_FF_FF;
            poison <= 1'b0;
        end
        else if (valid_data) begin
            // Continuously update the CRC state as data flows in.
            crcIn <= crcOut; 
            
            // Look for the End of Frame trigger
            if (eof) begin
                // The crcOut wire represents the calculated state AFTER this final byte.
                // We invert it (~crcOut) to match the IEEE 802.3 standard output.
                if (~crcOut == 32'h2144DF1C) begin //or (crcOut == h2144DF1C)
                    poison <= 1'b0; // Magic residue matches! Packet is clean.
                end
                else begin
                    poison <= 1'b1; // Residue is wrong. Packet is corrupted, assert poison!
                end
            end
        end
        else begin
            // When valid_data goes low (Inter-Packet Gap), clear everything for the next packet
            crcIn <= 32'hFF_FF_FF_FF;
            poison <= 1'b0;
        end
    end
