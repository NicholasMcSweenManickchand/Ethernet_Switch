module FCS(  // auto generated by bues.ch, since we look into the future to limit clock cycles (not shifting 8 times)
    input [7:0] data, // rx_data
    input clk, reset, valid_data, // tells us if we are ok to keep going or FIFO paused
    input done, // 9th bit of FIFO is 1
    input poison,
    output [31:0] rx_crc
);
    wire [31:0] crcOut;
   ...
   assign rx_crc = poison ? crcOut : ~crcOut;; //does not give the  correct output if poison
    ...
    always @(posedge clk) begin
        ...
        else if (!valid_data || done) begin
            crcIn <= crcIn;
        end
        else begin
            crcIn <= crcOut; // crcIn is the old crcOut
        end
    end
